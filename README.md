...
Run `make help` for Makefile targets and one-line descriptions. Those descriptions
live on each target recipe as a `## ` comment so they stay next to the code they
describe (and cannot drift the way a separately maintained table would).
